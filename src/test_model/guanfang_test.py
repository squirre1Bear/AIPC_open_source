...
def postprocess_file(file_path, score_dir):
    ...
    assert len(sage_parquet) == len(
        sage_score), f"{file_name}: parquet length ({len(sage_parquet)}) and score length ({len(sage_score)}) are inconsistent"
...
def main() -> None:
    ...
    if model_type in ('pth', 'bin', 'pt'):
        ...
        vocab = ['<pad>', '<mask>', 'A', 'D', 'E', 'F', 'G', 'H', 'I', 'K', 'L', 'M', 'N', 'O', 'P', 'Q', 'R', 'S', 'T',
                 'U', 'V', 'W', 'Y', 'C[57.02]', 'M[15.99]', 'N[.98]', 'Q[.98]', 'X', '<unk>']
        config["vocab"] = vocab
