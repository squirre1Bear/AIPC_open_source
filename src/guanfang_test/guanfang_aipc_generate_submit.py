...
def main():
    ...
    if args.type == 'basic':
        if len(basic_file_list) != 60:
            raise FileNotFoundError(f"Your pred files for the test dataset of basic track are not complete, we need 60, but you only have {len(basic_file_list)}")
    elif args.type == 'advanced':
        if len(advancd_file_list) != 50:
            raise FileNotFoundError(f"Your pred files for the test dataset of advanced track are not complete, we need 50, but you only have {len(advancd_file_list)}")
    else:
    ...
    if args.type == 'basic':
        target_fn_set = set([f'bas_a_testdata_{num}_benchmark_result.tsv' for num in range(30)]+[f'bas_b_testdata_{num}_benchmark_result.tsv' for num in range(30)])
        if set([i.split('/')[-1] for i in basic_file_list]) != target_fn_set:
            raise FilenameError("The name of your pred files should be bas_a_testdata_num_benchmark_result.tsv or bas_b_testdata_num_benchmark_result.tsv")
    elif args.type == 'advanced':
        target_fn_set = set([f'adv_a_testdata_{num}_benchmark_result.tsv' for num in range(30)]+[f'adv_b_testdata_{num}_benchmark_result.tsv' for num in range(20)])
        if set([i.split('/')[-1] for i in advancd_file_list]) != target_fn_set:
            raise FilenameError("The name of your pred files should be adv_a_testdata_num_benchmark_result.tsv or adv_b_testdata_num_benchmark_result.tsv")
    else:
        print('ERROR: your args type is wrong, should be basic or advanced')
        sys.exit()
    ...
    if args.type == 'basic':
        for file in basic_file_list:
            # 将tvs文件按df读入，并将df放入merged_list中
    ...
    # merged_list存储所有tsv文件对应的df格式数据，concat是将df纵向合并，ignore_index表示重新计算每行下标
