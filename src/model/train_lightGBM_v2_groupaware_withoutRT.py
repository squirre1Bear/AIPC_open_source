# python src/model/train_lightGBM_v2_groupaware_withoutRT.py \
#   --data-root /root/autodl-tmp/datasets/aipc \
#   --v1-model-dir ~/aipc/models/lgbm_v1_oof \
#   --out-dir ~/aipc/models/lgbm_v2_ranker_withoutRT \
...
from train_lightGBM_v1_folds_withoutRT import (
    CATEGORICAL_FEATURES,
    ID_COLUMNS_FOR_VALID,
    RT_QVALUE_ANOMALY_FEATURES,
    RT_QVALUE_ANOMALY_INSTRUMENTS,
    instrument_to_id_expr,
    list_split_files,
    mask_rt_qvalue_anomaly_features,
...
def load_v1_feature_columns(v1_model_dir: Path) -> List[str]:
    ...
    if not feature_file.exists():
        raise FileNotFoundError(f"??? v1 feature_columns.json: {feature_file}")
    with open(feature_file, "r", encoding="utf-8") as handle:
        feature_cols = json.load(handle)
    if not isinstance(feature_cols, list) or not feature_cols:
        raise RuntimeError(f"v1 feature_columns.json ????: {feature_file}")
...
def resolve_v2_feature_columns(files: List[Path], v1_feature_cols: List[str]) -> List[str]:
    ...
    base_cols = resolve_feature_columns(files)
    feature_cols = []
    ...
    if "lgbm_v1_score" not in deduped:
        raise RuntimeError("v2 ???? lgbm_v1_score????? add_group_feature_oof/test")
    return deduped
...
def ensure_aux_columns(df: pl.DataFrame) -> pl.DataFrame:
    ...
    if "instrument" not in df.columns:
        exprs.append(pl.lit(None).cast(pl.Utf8).alias("instrument"))
    if exprs:
...
def collect_group_dataset(
    ...
    instruments = [inst for inst in INSTRUMENTS if len(by_instrument.get(inst, [])) > 0]
    if not instruments:
        raise RuntimeError("没有可用于 group-aware 采样的仪器文件")
...
def train_model(args, feature_cols, train_df, valid_df, train_groups, valid_groups):
    ...
    train_features = mask_rt_qvalue_anomaly_features(
        train_features,
        train_df,
        feature_cols,
        context=f"groupaware {args.mode} train",
    )
    valid_features = mask_rt_qvalue_anomaly_features(
        valid_features,
        valid_df,
        feature_cols,
        context=f"groupaware {args.mode} valid",
    )
...
def main():
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=True,
        help="?????ranker ???????? scan ???? target ? decoy ? group?",
    )
    parser.add_argument(
        "--allow-uninformative-groups",
        action="store_true",
        help="?? informative-only ???????? target ?? decoy group?",
    )
