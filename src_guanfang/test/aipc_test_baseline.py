...
class Evalute(ptl.LightningModule):
    ...
    def test_step(
        ...
        # 挪到device上，并转换为torch.bfloat16类型，token在嵌入之后只能为long整型
        # 这几个变量用于模型预测，需要放到gpu上
    ...
    # 预测结束后记录到.cvs，包括index、score、label、weight四列
...
def postprocess_file(file_path, score_dir):
    ...
    # 先将score降序排列，之后再按scan_number去重，只保留第一行（分数最高的一个结果）
...
# python3 -m src_guanfang.test.aipc_test_baseline --model_path /zhangxiaofan/DDA_BERT_deltaRT/pred/mzml_tims_wiff.pt --parquet_dir data/bas_test_dataset --out_path result
